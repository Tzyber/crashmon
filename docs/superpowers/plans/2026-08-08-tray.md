...
pub fn pdeathsig_pre_exec(expected_ppid: u32) -> impl FnMut() + Send + 'static {
    move || {
...
`Cargo.toml` ergänzen (Testcode als eigenes Bin — kein env-Flag im Produktions-`main()`, k8-Lehre):
...
Expected: Fehler — `pdeath_helper`-Bin fehlt noch / compile-fehler.

- [ ] **Step 3: Bins/Tests zusammenbauen**
...
```rust
    ...
    tray_handle: Option<ksni::Handle<CrashmonTray>>,
...
```rust
        ...
        let (tray_active, tray_handle) =
            match ksni::TrayMethods::spawn(CrashmonTray::new(tray_tx)) {
...
- [ ] **Step 3:** Neuen Report erzeugen (Wedged-Test-Rezept aus README) → notify-send erscheint, Tray-Icon wird Alarm (rot)
- [ ] **Step 4:** Tray-Klick → Fenster erscheint (Fokus-Verhalten auf Wayland beobachten, dokumentieren wenn nur Blinken)
- [ ] **Step 5:** Tray-Menü „Beenden" → GUI + Daemon enden, `pgrep crashmon` leer
