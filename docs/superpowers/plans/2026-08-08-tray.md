...
/// `pre_exec` verlangt `FnMut + Send + Sync` — deshalb das `Sync`.
pub fn pdeathsig_pre_exec(expected_ppid: u32) -> impl FnMut() + Send + Sync + 'static {
    move || {
        // SAFETY: prctl/getppid sind async-signal-safe, im Kind nach fork
        // (exklusiver Kontext), kein Rust-Heap-Zugriff.
        unsafe {
            libc::prctl(libc::PR_SET_PDEATHSIG, libc::SIGTERM);
        }
        if unsafe { libc::getppid() } != expected_ppid {
            unsafe { libc::_exit(0) }; // Parent starb vor prctl -> Kind ohne Sinn
        }
    }
}
```

`spawn_daemon` erweitern:
...
pub fn spawn_daemon(cfg: &SpawnConfig) -> io::Result<Child> {
    ...
    std::process::Command::new(cfg.daemon_bin)
        ...
        // nutzt nur async-signal-safe Calls.
...
`src/lib.rs` (Bins sind separate Crates und sehen nur die Lib):

```rust
//! Library-Fassade fuer Test-Bins und Integrationstests: exportiert die
//! Pfade, die ausserhalb von main.rs gebraucht werden. Die GUI selbst
//! nutzt weiter `main.rs` + Module.
pub mod state;
```

Die übrigen Module (`app`, `config`, `format`, `logtail`, `reference`, `scan`, `tray`) bleiben in `main.rs` deklariert; `pdeath_helper` braucht nur `state`.
...
fn pdeathsig_killt_kind_beim_parent_tod() {
    ...
    let mut out = helper.stdout.take().unwrap();
    let deadline = Instant::now() + Duration::from_secs(5);
    loop {
        if out.read_line(&mut line).expect("PID-Zeile lesen") > 0 {
            break;
        }
    ...
    let mut alive = || unsafe { libc::kill(kind_pid as libc::pid_t, 0) } == 0;
    while alive() {
        assert!(
    ...
    // Cleanup: falls doch noch lebend, killen.
    if unsafe { libc::kill(kind_pid as libc::pid_t, 0) } == 0 {
...
- Produces:
  ...
  - `pub fn close_action(tray_active: bool, quitting: bool) -> CloseAction`
    - `(false, _)` → `Proceed`; `(true, false)` → `Hide`; `(true, true)` → `Proceed`
...
- Modify: `crashmon-gui/Cargo.toml` (ksni-dep), `crashmon-gui/src/lib.rs` (`pub mod tray;`), `crashmon-gui/src/main.rs` (`mod tray;` via lib: `use crashmon_gui::tray::{...}` — oder `mod tray;` im main belassen und in lib zusätzlich)

**Interfaces:**
- Produces:
  - `#[derive(Debug, PartialEq)] pub enum TrayCmd { Show, ToggleDaemon, Quit }`
...
fn menu_sendet_commands() {
    ...
    let MenuItem::Standard(fenster) = &menu[0] else { panic!("erstes Item") };
    assert_eq!(fenster.label, "Fenster anzeigen");
    (fenster.activate)(&mut tray);
    assert_eq!(rx.try_recv().unwrap(), TrayCmd::Show);

    let MenuItem::Standard(toggle) = &menu[1] else { panic!("zweites Item") };
    ...
    let MenuItem::Standard(quit) = &menu[2] else { panic!("drittes Item") };
...
fn toggle_label_und_foreign() {
    ...
    let MenuItem::Standard(toggle) = &tray.menu()[1] else { panic!() };
...
```rust
    ...
    tray_dirty: bool,
    /// Letzter Liveness-Check (Handle::is_closed, alle 2 s).
    last_tray_liveness: Instant,
```
...
+ Felder in Struct-Init (Produktion und `with_spawner`: `tray_active: false, tray_rx: mpsc::channel().1, tray_handle: None, hidden: false, quitting: false, tray_dirty: false, last_tray_liveness: Instant::now()`).
...
```rust
        // Tray-Telegramme (Show/Quit/Toggle)
        while let Ok(cmd) = self.tray_rx.try_recv() {
            for c in self.handle_tray_cmd(cmd) {
                ctx.send_viewport_cmd(c);
            }
        }
        // Tray-Liveness: is_closed() ist lokal (kein D-Bus-Roundtrip),
        // alle 2 s — NICHT im 500-ms-Tick. Host weg -> Fenster wieder
        // zeigen, sonst waere die GUI unerreichbar (Statuszeile im
        // versteckten Fenster sieht niemand).
        if self.tray_active && self.last_tray_liveness.elapsed() >= Duration::from_secs(2) {
            self.last_tray_liveness = Instant::now();
...
Neues Feld `just_spawned: bool` (in `mount()` nach Erfolg `true`, von `logic()` konsumiert) — TOCTOU-Nachlauf:
...
- [ ] **Step 7:** systemd-Unit aktiv + GUI-Start → „läuft extern", kein zweiter Daemon, Reports live
- [ ] **Step 8:** D-Bus-Host killen → Fenster erscheint wieder (Tray-Verlust-Pfad)
